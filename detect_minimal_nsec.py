...
def label_count(name, zone):
    """Number of labels in name relative to zone (excluding root)."""
    rel = name.relativize(zone)
    return len([l for l in rel.labels if l])


def is_synthetic_label(label):
    """
    Check if a DNS label looks like it was generated by an epsilon
    function rather than being a real zone name. Synthetic child labels
    used as successors are typically a single non-alphanumeric byte
    (e.g., \x00, '!').
    """
    if len(label) > 2:
        return False
    return all(not (0x30 <= b <= 0x39 or 0x41 <= b <= 0x5A
                    or 0x61 <= b <= 0x7A)
               for b in label)


def check_next_depth(qname, next_name, zone):
    """
    Check if the NSEC next name has a plausible depth for an epsilon
    successor. An epsilon successor is either a sibling of the query
    name (same depth) or a direct child with a synthetic label (one
    deeper). Returns False if the next name is deeper than expected
    or has a non-synthetic child label.
    """
    q_depth = label_count(qname, zone)
    next_depth = label_count(next_name, zone)

    if next_depth <= q_depth:
        return True

    if next_depth == q_depth + 1:
        rel = next_name.relativize(zone)
        innermost = rel.labels[0]
        return is_synthetic_label(innermost)
...
def analyze_nsec(qname, owner, next_name, zone):
    ...
    next_depth_ok = check_next_depth(qname, next_name, zone)
...
def cmd_calc(args):
    ...
    if args.qname:
        ...
        depth_str = "yes" if info['next_depth_ok'] else "NO"
        print(f"    next depth ok:    {depth_str}")

        if (info['next_ratio'] >= 0.75 and info['owner_ratio'] >= 0.5
                and info['next_depth_ok']):
            print(f"\n  => NSEC boundaries track the query name")
            print(f"     Consistent with minimally covering NSEC"
                  f" (epsilon function)")
        elif info['next_ratio'] >= 0.5 and info['next_depth_ok']:
...
def cmd_probe(args):
    ...
    all_depth_ok = all(r['next_depth_ok'] for r in results)
    ...
    print(f"  Next depth ok:        {'all' if all_depth_ok else 'FAIL'}")

    if min_next >= 0.75 and avg_owner >= 0.5 and all_depth_ok:
        print(f"\n  => NSEC boundaries consistently track query names")
        print(f"     Likely minimally covering NSEC (epsilon function)")
    elif min_next >= 0.5 and all_depth_ok:
